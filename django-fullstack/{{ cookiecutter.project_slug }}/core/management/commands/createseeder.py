...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "app_model", type=str, help="Nome no formato App.Model (ex: auth.User)"
        )
        parser.add_argument(
            "--quantity",
            type=int,
    ...
    def handle(self, *args, **options):
        try:
            ...
            raise CommandError(
                "Formato inválido. Use App.Model (ex: auth.User)"
            ) from exc
        ...
        try:
            ...
            raise CommandError(
                f"Modelo {model_name} não encontrado no app {app_label}"
            ) from exc
        ...
        if options["update"] and os.path.exists(seeder_file):
            with open(seeder_file) as f:
                content = f.read()
                data_match = re.search(
                    r"# SEED DATA START\n(.*?)# SEED DATA END", content, re.DOTALL
                )
                if data_match:
                    existing_data = data_match.group(1).strip().split("\n")

        seeder_content = self.generate_seeder_content(
            model, options["quantity"], options["app_model"], existing_data
        )
    ...
    def generate_seeder_content(self, model, quantity, model_name, existing_data=None):
        """Gera o conteúdo do arquivo seeders.py dinâmico"""
        model_name = model.__name__
        app_label = model._meta.app_label

        unique_fields, defaults_fields, _fk_fields = self.analyze_model_fields(model)
...
"""
    ...
    def get_field_example(self, field, field_type):
        ...
        examples = {
            "CharField": f"Texto {field_name}"
            + (f" (max {max_length})" if max_length else ""),
