...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "seeder_name", nargs="?", type=str, help="Nome do seeder específico"
        )
        parser.add_argument(
            "--all", action="store_true", help="Executar todos os seeders"
        )
        parser.add_argument(
            "--check", action="store_true", help="Verificar status sem executar"
        )
        parser.add_argument(
            "--force", action="store_true", help="Forçar execução mesmo se já aplicado"
        )
    ...
    def run_app_seeders(self, app_name, seeder_name, options):
        ...
        seeder_files = [
            f for f in os.listdir(app_dir) if f.endswith(".py") and f != "__init__.py"
        ]
    ...
    def execute_seeder(self, app_dir, seeder_file, options):
        ...
        if (
            not created
            and execution.data_hash == data_hash
            and not options.get("force")
        ):
            self.stdout.write(
                self.style.WARNING(
                    f"✓ {seeder_name} já foi aplicado com os dados atuais. "
                    "Use --force para reexecutar"
                )
            )
            return
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✓ {seeder_name} executado e registrado com sucesso"
                )
            )

        except Exception as e:
            raise CommandError(f"Erro ao executar {seeder_name}: {e}") from e
    ...
    def calculate_data_hash(self, app_dir, seeder_name):
        ...
        seeder_json = os.path.join(
            data_dir, f"{seeder_name.replace('_seeder', '')}.json"
        )
